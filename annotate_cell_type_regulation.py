...
def annotate_cell_types_vector(nucleotides_db, annotations_db, reference_genome, chromosome, start_position, end_position):
    ...
    :return: a 3-long list:
            list[0] = sum_score in format numpy.float64 (mean score)
            list[1] = mean_score in format numpy.float64 (mean score)
            list[2] = 164 values from 0 to 9 corresponding to the label for each cell type in format numpy.ndarray with shape (164,) and dtype=numpy.uint8. (max score for each cell type)
